...
frame1 <- cda_v(data, n_frame:nf, nc, nr, v_max, n_sm)
frame <- matrix(frame1[,1], nrow = nr, ncol = nc, byrow = FALSE)

#frame <- cda(data, n_frame, nc, nr, v_max, n_sm)

#frame1 <- as.numeric(data[n_frame, 1:(nc*nr)])
#frame <- matrix(frame1, nrow = nr, ncol = nc, byrow = TRUE)
